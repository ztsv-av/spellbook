...
def saveTrainInfo(model_name, epoch, train_loss, train_accuracy, val_loss, val_accuracy, optimizer, save_csvs_dir):
    ...
    info_df = pd.DataFrame({
        'epoch': [epoch + 1], 'learning_rate': [optimizer._decayed_lr(tf.float32).numpy()], 'train_loss': [train_loss.numpy()], 
        'train_accuracy': [(train_accuracy.result() * 100).numpy()], 'val_loss': [val_loss.result().numpy()], 
...
def saveTrainInfoDetection(model_name, epoch, loc_loss, class_loss, total_loss, optimizer, save_csv_dir):
    ...
    info_df = pd.DataFrame({
        'epoch': [epoch + 1], 'learning_rate': [optimizer._decayed_lr(tf.float32).numpy()], 
        'loc_loss': [loc_loss.numpy()], 'class_loss': [class_loss.numpy()], 'total_loss': [total_loss.numpy()]})
